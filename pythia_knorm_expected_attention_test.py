...
class Result:
    ...
    expected_attention_compression_ratio: float
    ppl: float
...
def build_press(
    name: str,
    knorm_ratio: float,
    expected_attention_ratio: float,
    expected_attention_future_positions: int,
    expected_attention_sink_tokens: int,
    expected_attention_no_covariance: bool,
    expected_attention_no_vnorm: bool,
):
    if name == "none":
        return NoPress()

    knorm = KnormPress(compression_ratio=knorm_ratio)
    expected_attention = ExpectedAttentionPress(
        compression_ratio=expected_attention_ratio,
        n_future_positions=expected_attention_future_positions,
        n_sink=expected_attention_sink_tokens,
        use_covariance=not expected_attention_no_covariance,
        use_vnorm=not expected_attention_no_vnorm,
    )
    ...
    if name == "knorm_expected_attention":
        return ComposedPress([knorm, expected_attention])
...
def run(args) -> list[Result]:
    ...
    for dataset_name in args.datasets:
        ...
        for press_name in args.presses:
            ...
            press = build_press(
                name=press_name,
                knorm_ratio=args.knorm_compression_ratio,
                expected_attention_ratio=args.expected_attention_compression_ratio,
                expected_attention_future_positions=args.expected_attention_future_positions,
                expected_attention_sink_tokens=args.expected_attention_sink_tokens,
                expected_attention_no_covariance=args.expected_attention_no_covariance,
                expected_attention_no_vnorm=args.expected_attention_no_vnorm,
            )
            ...
            print(
                f"{press_name:>24}: ppl={ppl:.4f}, throughput={throughput:.2f} tokens/s, "
            ...
            results.append(
                Result(
                    ...
                    expected_attention_compression_ratio=args.expected_attention_compression_ratio,
                    ppl=ppl,
...
def parse_args():
    ...
    parser.add_argument(
        "--presses",
        nargs="+",
        default=["none", "knorm", "expected_attention", "knorm_expected_attention"],
        choices=["none", "knorm", "expected_attention", "knorm_expected_attention"],
    )
    ...
    parser.add_argument("--expected-attention-compression-ratio", type=float, default=0.25)
    parser.add_argument("--expected-attention-future-positions", type=int, default=512)
    parser.add_argument("--expected-attention-sink-tokens", type=int, default=4)
    parser.add_argument("--expected-attention-no-covariance", action="store_true")
